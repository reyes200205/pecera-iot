...
export const getAguaData = async (deviceID: string , res: Response) => {
    try {  
      if (!deviceID) {
        return res.status(400).send("Error: Device ID not provided");
      }
  
      const topic = `pecera-${deviceID}-agua`;
      ...
      if (response.ok) {
        ...
        res.json({message: "Data fetched successfully", data: dataSensor.toString()});
      } else {
